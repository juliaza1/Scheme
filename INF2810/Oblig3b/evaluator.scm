...
(define (eval-special-form exp env)
  (cond ((quoted? exp) (text-of-quotation exp))
        ...
        ((new_if? exp) (eval-new_if exp env))      ;; 3b
        ((lambda? exp)
         (make-procedure (lambda-parameters exp)
                         (lambda-body exp)
                         env))
        ((let? exp) (let->lambda exp env))         ;; 3c
        ((new_let? exp) (new_let->lambda exp env)) ;; 3d
        ((begin? exp) 
...
;;__________________________________________       2b 3a 3b 3c 3d 3e       ________________________________________________________________________________
;;______________________________________________________________________________________________________________________________________________
;; 2b)
(define (install-primitive! proc-name proc)
  (begin (set! primitive-procedures    ;; legger til proc-name og proc i listen primitive-procedures
               (cons (list proc-name proc) primitive-procedures))
         (set! the-global-environment  ;; oppdaterer omgivelsen slik at den også inneholder den nye procedyren
               (extend-environment (primitive-procedure-names)
                                   (primitive-procedure-objects)
  ...
  (define (iter x)
    (if (null? x)
        ...
        (if (car x)
            (iter (cdr x))
            #f)))
  (iter (list-of-values (rest-exps exps) env)))
